...
function countdown() {
    ...
    var launch_date = new Date(Date.UTC(2019, 4, 25, 0, 0));
    var days;
    var hours;
    var minutes;
    var seconds;
    ...
    ctd.innerHTML =
        '<li>' + days + '<span> дн' + (days > 1 ? 'ей' : '') + '</span></li>' +
        '<li>' + hours + '<span> час' + (hours > 1 ? 'ов' : '') + '</span></li>' +
        '<li>' + minutes + '<span> минут' + (minutes == 1 ? 'а' : '') + '</span></li>' +
        '<li>' + seconds + '<span> секунд' + (seconds == 1 ? 'а' : '') + '</span></li>';
